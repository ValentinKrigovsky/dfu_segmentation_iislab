...
def convert_2d_segmentation_nifti_to_img(nifti_file: str, output_filename: str, transform=None, export_dtype=np.uint8, dfu_b_w = False):
    img = sitk.GetArrayFromImage(sitk.ReadImage(nifti_file))
    assert img.shape[0] == 1, "This function can only export 2D segmentations!"
    img = img[0]

    # # Delete this block, set by V.krigovsky for experimenting
    if dfu_b_w:
        img[img >= 1] = 255
...
def convert_2d_segmentation_nifti_to_img_not_save(nifti_file: str, transform=None, export_dtype=np.uint8):
    img = sitk.GetArrayFromImage(sitk.ReadImage(nifti_file))
    assert img.shape[0] == 1, "This function can only export 2D segmentations!"
    img = img[0]

    # # Delete this block, set by V.krigovsky for experimenting
    # rows, cols = img.astype(export_dtype).shape
    # for i in range(rows):
    #     for j in range(cols):
    #         k = img[i, j]
    #         if k == 1:
