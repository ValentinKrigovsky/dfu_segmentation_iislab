...
def save_segmentation_nifti_from_softmax(segmentation_softmax: Union[str, np.ndarray], out_fname: str,
    ...
    mask_array_converted_for_dfu_challenge = seg_old_size_postprocessed.astype(np.uint8)
    mask_array_converted_for_dfu_challenge[mask_array_converted_for_dfu_challenge > 0] = 255
    seg_resized_itk = sitk.GetImageFromArray(mask_array_converted_for_dfu_challenge)
    seg_resized_itk.SetSpacing(properties_dict['itk_spacing'])
    seg_resized_itk.SetOrigin(properties_dict['itk_origin'])
    seg_resized_itk.SetDirection(properties_dict['itk_direction'])
    # .replace("nii.gz", "png") Is customization added to save data as png for 2d challenge!
    sitk.WriteImage(seg_resized_itk, out_fname.replace("nii.gz", "png"))
    ...
    if (non_postprocessed_fname is not None) and (seg_postprogess_fn is not None):
        ...
        seg_resized_itk.SetDirection(properties_dict['itk_direction'])
        # .replace("nii.gz", "png") Is customization added to save data as png for 2d challenge!
        sitk.WriteImage(seg_resized_itk, non_postprocessed_fname.replace("nii.gz", "png"))
